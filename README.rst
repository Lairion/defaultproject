...
``-f``, ``--format``       Format to use when generating views and urls. Valid options: ``viewset``, ``apiview``, ``function``, ``modelviewset``, ``genericmodelviewset``. Default: ``viewset``.
...
``python manage.py generate api --format genericmodelviewset``
