...
import os.path
import django
from drfg.templates.serializer import SERIALIZER
from drfg.templates.apiview import API_URL, API_VIEW, API_URL_V2
from drfg.templates.viewset import VIEW_SET_URL, VIEW_SET_VIEW, VIEW_SET_URL_V2
from drfg.templates.function import FUNCTION_URL, FUNCTION_VIEW, FUNCTION_URL_V2
from drfg.templates.modelviewset import MODEL_URL, MODEL_VIEW, MODEL_URL_V2
from drfg.templates.genericviewset import GENERIC_MODEL_VIEW, GENERIC_MODEL_URL, GENERIC_MODEL_URL_V2

__all__ = ['BaseGenerator', 'APIViewGenerator', 'ViewSetGenerator',
           'FunctionViewGenerator', 'ModelViewSetGenerator','GenericModelViewSetGenerator']
class BaseGenerator(object):
    ...
    def write_file(self, content, filename):
        try:
            ...
            print('Please create "api" folder')


class APIViewGenerator(BaseGenerator):

    def __init__(self, app_config, force):
        api_url = API_URL_V2 if django.VERSION[0]>=2 else API_URL
...
class ViewSetGenerator(BaseGenerator):
    ...
    def __init__(self, app_config, force):
        view_set_url = VIEW_SET_URL_V2 if django.VERSION[0]>=2 else VIEW_SET_URL
...
class FunctionViewGenerator(BaseGenerator):
    ...
    def __init__(self, app_config, force):
        function_url = FUNCTION_URL_V2 if django.VERSION[0]>=2 else FUNCTION_URL
...
class ModelViewSetGenerator(BaseGenerator):
    ...
    def __init__(self, app_config, force):
        model_url = MODEL_URL_V2 if django.VERSION[0]>=2 else MODEL_URL
...
class GenericModelViewSetGenerator(BaseGenerator):

    def __init__(self, app_config, force):
        generic_model_url = GENERIC_MODEL_URL_V2 if django.VERSION[0]>=2 else GENERIC_MODEL_URL
        self.view_template = Template(GENERIC_MODEL_VIEW)
        self.url_template = Template(generic_model_url)
        super(GenericModelViewSetGenerator, self).__init__(app_config, force)